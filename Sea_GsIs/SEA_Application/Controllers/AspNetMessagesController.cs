    ...
    public class AspNetMessagesController : Controller
    {
        ...
        public ActionResult NewMessage()
        {
          ...
          //  numbers.Add("923401562576");
            ...
            SendSMS(tyu, numbers);
            ViewBag.ReceiverList = new SelectList(db.AspNetUsers.Where(x => x.AspNetRoles.Select(y => y.Name).Contains("Branch_Admin")).ToList(), "Id", "UserName");
        ...
        public JsonResult GetClasses()
       {
            var aspNetClasses = (from classes in db.AspNetClasses
                                select new { classes.Id, classes.Name }).ToList();
            return Json(aspNetClasses, JsonRequestBehavior.AllowGet);
      }

        public JsonResult GetStudents()
        {
            var Students = db.AspNetUsers.Where(x => x.AspNetRoles.Select(y => y.Name).Contains("Student") && x.StatusId != 2).Select(x => new { x.Id, x.Name, x.UserName }).ToList();
            return Json(Students, JsonRequestBehavior.AllowGet);
        }

        public JsonResult GetTeachers()
        {
            var Teachers = db.AspNetUsers.Where(x => x.AspNetRoles.Select(y => y.Name).Contains("Teacher") && x.StatusId != 2).Select(x => new { x.Id, x.Name, x.UserName }).ToList();
            return Json(Teachers, JsonRequestBehavior.AllowGet);
        }

        public JsonResult GetParents()
        {
            var Parents = db.AspNetUsers.Where(x => x.AspNetRoles.Select(y => y.Name).Contains("Parent") && x.StatusId != 2).Select(x => new { x.Id, x.Name, x.UserName }).ToList();
        ...
        public JsonResult GetStudentsByClass(string Classes)
        {
            ...
            if (Classes != "null")
            {
                ...
                var Students = (from students in db.AspNetStudents
                                where selectClass.Contains(students.ClassId) && students.AspNetUser.StatusId != 2
        ...
        {
            var Users = db.AspNetUsers.Where(x => x.StatusId != 2).Select(x => new { x.Id, x.Name, x.UserName }).ToList();
