...
namespace SEA_Application.Controllers
{
    public class EmployeeSalariesController : Controller
    {
        ...
        {
            var employeeSalaries = db.EmployeeSalaries.Include(e => e.AspNetSession).Include(e => e.AspNetEmployee);
        ...
        public ActionResult Increment()
        {
         //   ViewBag.EmpSalaryRecordId = new SelectList(db.AspNetEmployees.Where(x => x.AspNetUser.Status != "False"), "Id", "Name");
            ViewBag.EmpSalaryRecordId = new SelectList(from emp in db.AspNetEmployees
                                                join usr in db.AspNetUsers on emp.UserId equals usr.Id
                                                where usr.StatusId != 2
                                                select new { emp.Id, emp.Name }, "Id", "Name");
            return View();
        }
        ...
        public ActionResult Create()
        {
          ...
          //  ViewBag.EmployeeId = new SelectList(db.AspNetEmployees.Where(x => x.AspNetUser.Status != "False"), "Id", "Name");
            ViewBag.EmployeeId = new SelectList(from emp in db.AspNetEmployees
                                                join usr in db.AspNetUsers on emp.UserId equals usr.Id
                                                where usr.StatusId != 2
                                                select new { emp.Id, emp.Name }, "Id", "Name");
            return View();
        ...
        public ActionResult GetEmployeeSalary(int empid)
        {
            ...
            var fine = db.EmployeePenalties.Where(x => x.EmployeeId == empid && x.Status == "Created").Select(x => x.EmployeePenaltyType.Amount).FirstOrDefault();
            if (fine == null)
            {
                fine = 0;
            }
        ...
        public ActionResult Create([Bind(Include = "Id,EmployeeId,Salary,Months,Date,Status,SessionId")] EmployeeSalary employeeSalary)
        {
            ...
            ViewBag.SessionId = new SelectList(db.AspNetSessions, "Id", "Year", employeeSalary.SessionId);
